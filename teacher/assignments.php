...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        $stmt->bind_param('ssissi', $title, $desc, $bid, $due, $file_path, $uid); $stmt->execute();
        $msg = '<div class="alert alert-success">Assignment created!</div>';
    } elseif ($action === 'grade') {
        $sub_id = (int)$_POST['submission_id']; $marks = floatval($_POST['marks']);
        $conn->query("UPDATE submissions SET marks=$marks, status='graded' WHERE id=$sub_id"); 
        $msg = '<div class="alert alert-success">Graded!</div>';
    }
}
...
<div class="table-card">
    <div class="table-responsive">
        <table>
            <thead><tr><th>Title</th><th>Batch</th><th>Due Date</th><th>Submissions</th><th>File</th></tr></thead>
            <tbody>
                <?php if ($assignments->num_rows === 0): ?><tr><td colspan="5" style="text-align:center;padding:30px;color:var(--text-secondary);">No assignments yet.</td></tr><?php else: ?>
                ...
                <tr>
                    ...
                    <td><?= $a['due_date'] ? date('M d, Y H:i', strtotime($a['due_date'])) : 'No deadline' ?></td>
